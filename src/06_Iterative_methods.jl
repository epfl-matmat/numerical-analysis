...
\qquad k = 0, 1, \ldots,
```
that is in each step we solve the linear system $\mathbf{P} \mathbf{x}^{(k+1)} = \widetilde{\mathbf{b}}^{(k)}$ where $\widetilde{\mathbf{b}}^{(k)} = (\mathbf{P} - \mathbf{A}) \mathbf{x}^{(k)} + \mathbf{b}$.
"""
...
Notice that here we employed the short-hand $g'$ to denote the Jacobian matrix $\mathbf{J}_g$ of the fixed-point map $g$.
"""

# ╔═╡ ba77a115-f9fb-4f80-a0ea-eb50ddd9ae65
md"""
Recall that our condition for convergence was $\|g'(\mathbf{x}_\ast) \| < 1$,
which for the case of Richardson iterations is thus $\| \mathbf B \| < 1$.
...
# ╔═╡ bfeaa46f-7abb-43f5-b6f0-f9b4a1637326
md"""
...
# ╔═╡ 40ba9ce5-d6b0-4a4f-8dc7-1e955c9f3af9
Foldable("Explicit verification the Taylor expansion terminates",
md"""
We verify that the Taylor expansion terminates exactly by inserting our expressions
for $g(\mathbf x_\ast)$ and $g'(\mathbf x_\ast)$ into the  right-hand side:
```math
\begin{aligned}
...
# ╔═╡ 380aff35-1dd0-4dc7-afac-099448350135
md"""
Denoting the error in the $k$-th iteration as
$\mathbf{e}^{(k)} = \mathbf{x}^{(k)} - \mathbf{x}_\ast$
and subtrtacting $\mathbf{x}_\ast$ from the above Taylor expansion
...
md"""
!!! danger "Difference between linear and non-linear functions g"
	Notice that the result (4) and Theorem 1 of this notebook only
	applies for fixed-point problems with a *linear* function $g$.
	This is therefore a different setting to
	the Convergence analysis discussion in [Root finding and fixed-point problems](https://teaching.matmat.org/numerical-analysis/04_Nonlinear_equations.html),
	where we discussed the more general case of a *non-linear* fixed point functions $g$.

	For non-linear $g$ we can only state $\mathbf{e}^{(k+1)} = g'(\mathbf{x}_\ast) \mathbf{e}^{(k)} + O(\text{small})$, where the small term has a size that is quadratic in $\|\mathbf{e}^{(k)}\|$.
	Therefore an exact equality like (4) does not hold,
	which in particular means that we need the higher-order terms
	hidden in $O(\text{small})$ to be sufficiently small in order for
	the error to actually shrink going from $k$ to $k+1$ (implying convergence).
	This is only ensured if $\| \mathbf{e}^{(k)}\|$ is small enough, i.e. if one starts the iterations sufficiently close to the fixed point $x_\ast$.
	This is why in Theorem 1 and Theorem 3
	of [Root finding and fixed-point problems](https://teaching.matmat.org/numerical-analysis/04_Nonlinear_equations.html)
	we have the condition that $x^{(0)}$ needs to be taken from an interval
	around $x_\ast$.
"""

# ╔═╡ 318167c9-719a-4258-97c5-e071598a38d8
md"""
...
represents only $1/10$ times the actual gradient.
Therefore simply
...
manifest_format = "2.0"
project_hash = "4e0b25cd69a3bfccab8e5d4517803188c90510c4"
