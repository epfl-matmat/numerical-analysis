...
However, elementary floating-point operations
(single addition, multiplication, evaluation of square root function, ...)
furthermore come with the guarantee that
```math
|\epsilon_i| ≤ \frac{\epsilon_M}{2}
```
For more details see the discussion in the [Introduction](https://teaching.matmat.org/numerical-analysis/01_Introduction.html#Structure-of-floating-point-numbers).
