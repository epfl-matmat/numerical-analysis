...
The point of this lecture is now to formalise such numerical procedures mathematically,
...
You might think learning about this is a bit of an overkill and probably not very useful for your future studies. But let us look at two examples, which hopefully relate to problems you already solved as part of your studies. Both of them we will pick up during the lectures and expand upon in more details ... and in both cases we will realise that finding an efficient and reliable numerical technique for solving them can sometimes be surprisingly tricky.
"""
...
begin
	...
	# Scatter plot
	scatter(temperature, rate, label="Observed rates", xlabel=L"T", ylabel=L"k")
end
...
# ╔═╡ 62396307-b329-4855-9467-b01a2b46b78e
md"""
...
The problem is that for many realistic numerical settings (e.g. the quantum-chemical simulation of materials) even standard packages such as NLsolve are no longer able to automatically figure out the best solution strategy. As a result developing some intuition for the required numerical procedure as well as developing some tools to measure their reliability is necessary to:
- realise you have encountered a numerical problem, when your numerical approach gives you unreasonable answers,
- develop some ideas how to overcome these numerical issues;

Even if you now might think, that your heart beats for experimental research and you will never need numerics, keep in mind that all data you analyse uses procedures based on the techniques we will discuss. Even commercial packages for data analysis or experimental post-processing rely heavily on the procedures we will cover. Unfortunately even the most basic tools for data analysis (such as [Excel](https://www.economist.com/graphic-detail/2016/09/07/excel-errors-and-science-papers)) have in the past had issues, which **have compromised scientific fundings**. So surely it's best to be prepared, so you can judge what is wrong: The experiment or the numerics.
"""

# ╔═╡ f6caf825-ca81-4e07-b143-a665ed9d93d4
md"""
...
I can also recommend taking a look at the lecture notes [**First Semester Numerical analysis**](https://diginole.lib.fsu.edu/islandora/object/fsu%3A657877/datastream/PDF/view) **by Giray Ökten**. At places the text is more mathematical than our treatment, but in particular it has a good summary of basic Julia commands an concepts. [An online PDF](https://diginole.lib.fsu.edu/islandora/object/fsu%3A657877/datastream/PDF/view) is available.
...
manifest_format = "2.0"
project_hash = "afac85479bf92221889836c3561be1cd29cbfa22"
