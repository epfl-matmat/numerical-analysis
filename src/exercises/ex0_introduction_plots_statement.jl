...
- `color` or `c`: Changes the color of the series.
- `linestyle` or `ls`: Changes the style of the line. For example `:solid` or `:dot`.
- `linewidth` or `lw`: Width of the line in pixels. We will often use `lw=2` since the default 1 is quite thin.
- `markerstyle` or `mark`: Changes the style of markers, for example `:o` for round markers. We will often use `mark=:o` to see the individual data points in a line plot.
...
# ╔═╡ 68a6e6c9-6097-47bc-bf69-d023a4fbd5fa
let
	xs = range(0, π; length=100)
	plot(xs, cos.(xs); label="")
	plot!(; xlabel=L"x", ylabel=L"\sin(x)", title=L"$\sin(x)$ between $0$ and $\pi$")
end
...
manifest_format = "2.0"
project_hash = "9873fadc80205931a7aa0c9d128331aa9541df9c"
